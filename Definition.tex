...
\subsubsection{eigene Synthese unter Verwendung der Strategie-Definition von Henry Mintzberg} % soll ich das anders benennen???
...
Die letzte Art, die systematische Beratung ist aus der neueren Systemtheorie entstanden. Der Kunde wird unterstützt \glqq bei seiner Selbstreflexion\grqq. 

Die Begriffe werden in der Literatur auch teilweise einzeln und eigenständig definiert.

In \cite[28]{MuellerNagel} hingegen findet man eine weitere Definition für Organisationsberatung (eine Definition für Prozessberatung fehlt) . Demnach ist Organisationsberatung eine Beratung die \glqq  sich auf menschliche Kollektive bezieht, die ihr Verhalten auf gewisse Weise miteinander abstimmen (koordinieren) und so eine Handlungsrealität herstellen, die – mehr oder weniger – funktional ist. Diese Kollektive befinden sich im ständigen Prozess des Organisierens und versuchen so, Probleme zu lösen, die zur Aufrechterhaltung ihres Daseinszwecks gelöst werden müssen. Immer wieder wird aber entweder genau dieser Prozess zum eigentlichen Problem, das dann mit externer Hilfe in Form von Organisationsberatung gelöst werden soll, oder der Daseinszweck steht in Frage und drängt nach neuen Antworten.
\qrqq Hier wird der Begriff also aus dem sozialen Kollektiv hergeleitet, dass einen bestimmten Daseinszweck besitzt der sich aus bestimmten Problemen ableitet.
...
\subsection{IT-Beratung}
	\subsubsection{Grund für den Zuwachs von IT-Beratung}
		...
		\subsubsection{IT-Consulting als prozessbezogene Beratungsart mit Fokus auf Systementwicklung und -integration}
		...
		\subsubsection{ IT-Consulting als  projektbezogene Unterart des Consultings im IT-Umfeld}
